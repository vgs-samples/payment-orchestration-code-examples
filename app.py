...
def checkout(): 
    ...
    proxies = {
        'https': f'http://{CUSTOMER_VAULT_ACCESS_CREDS_USERNAME}:{CUSTOMER_VAULT_ACCESS_CREDS_SECRET}'
                 f'@{PAYMENT_ORCH_APP_DOMAIN}:8080',
    }
    headers = {
    ...
    fin_instr = requests.post(
        f'https://{PAYMENT_ORCH_APP_DOMAIN}/financial_instruments',
        headers=headers,
        json=fin_instr_data,
        proxies=proxies,
    ...
    transfer = requests.post(
        f'https://{PAYMENT_ORCH_APP_DOMAIN}/transfers',
        headers=headers,
        json=transfers_data,
        proxies=proxies,
        verify=Path(__file__).resolve().parent / f'certs/sandbox_cert.pem'
